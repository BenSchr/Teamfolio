...
@app.get("/user/{user_id}/collected", response_model=List[UserReduced])
async def create_collection_by_userid(user_id: int, db: Session = Depends(get_db)):
    return get_collected_by_userid(db, user_id=user_id)
...
async def create_collection(collection: Collection, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=400, detail="Could not create user", headers={
                            "X-Error": "There goes my error"})


@app.delete("/user/{user_id}/collection/delete")
async def delete_collection(user_id: int, db: Session = Depends(get_db)):
    response = db.query(models.Collection).filter(
        models.Collection.user_id_aktiv == user_id).delete()
    db.commit()
...
async def delete_erfolg(user_id: int, db: Session = Depends(get_db)):
    response = db.query(models.Erfolg_User).filter(
        models.Erfolg_User.user_id == user_id).delete()
    db.commit()
    return {"numDelRows": response}
...
async def check_achievements(user_id: int, db: Session = Depends(get_db)):
    ...
    new_achievements = achievement_helper(
        [x for x in achievements if x["gesammelt"] == 0], collection_size)
    results = []
    for new in new_achievements:
        results.append(create_erfolg_user_entry(db, Erfolg_User.parse_obj(
            {"user_id": user_id, "erfolg_id": new["erfolg_id"]})))
...
@app.get("/erfolge/stats",response_model=List[Erfolg_stats])
async def get_achievements_user(db: Session = Depends(get_db)):
    return calculate_achievement_stats(db)
